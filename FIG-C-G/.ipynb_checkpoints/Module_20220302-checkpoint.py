...
def make_streamplot(equations, min_var = 0,  max_var = 15 ,var_step = 1,  E_slice=1, file_label = 0):
    ...
    strm = plt.streamplot(var_grid[0], var_grid[1], xdot, ydot, density=1.5, color='darkblue')
    plt.title("Streamplot")
    plt.xlabel('Gata6 Concentration')
    plt.ylabel('Nanog Concentration')
    plt.savefig(str(file_label) + '_streamplot_den1.svg')
    

def make_streamplot_log(equations, min_var = 0,  max_var = 100 ,var_step = 1,  E_slice=1, file_label = 0):
    ...
    fig = plt.figure(figsize=(8,8))
    strm = plt.streamplot(var_grid[0], var_grid[1], xdot, ydot, density=5, color='darkblue')
    plt.xscale('log')
    plt.yscale('log')
    plt.title("Streamplot")
    plt.xlabel('Gata6 Concentration')
...
def plot_bifurcation(table_all_ic):
    ...
    ax1.set(xlabel = 'FGF', ylabel = 'GATA6')
    ax1.set_ylim(-1,18)
...
def plot_bifurcation_v4(table_all_ic, file_label = 0):
    '''Takes the tbale with columns FGF, var1(tmax), var2(tmax) ... and plots the var(tmax) vs FGF'''
    fig, (ax1, ax2, ax3) = plt.subplots(1, 3, sharey=True, figsize = (12,4)) #figsize = (6,2)
    fig.suptitle('bifurcation')
    
    ax1.semilogx(table_all_ic[:,0],table_all_ic[:,1], '_', color='darkred', markersize = 6, alpha = 0.2)
    ax1.set(xlabel = 'FGF', ylabel = 'GATA6')
    ax1.set_ylim(-1,18)
    ax1.set_xticks(ticks = np.logspace(-1,1,10, base = 2 ), minor=False)

    ax2.semilogx(table_all_ic[:,0],table_all_ic[:,2], '_', color='darkgreen', markersize = 6, alpha = 0.2)
    ax2.set(xlabel = 'FGF', ylabel = 'NANOG')
    
    ax3.semilogx(table_all_ic[:,0],table_all_ic[:,3], '_', color='darkblue', markersize = 6, alpha = 0.2)
    ax3.set(xlabel = 'FGF', ylabel = 'ESRRB')    
    
    fig.savefig(str(file_label)+'_bifurcationplot_1000.png', dpi = 1000)
...
def plot_bifurcation_v3(table_all_ic):
    ...
    ax1.semilogx(table_all_ic[:,0],table_all_ic[:,1],'*r', markersize = 3, alpha = 1)
...
def sample_trajectories(ic, parameters, equations, tmax_int = 50, tmax_ss = 200, verbose = False, E_slice = 0, file_label = 0):
    ''' Integrates ODE's specified by "equations" and returns values of variables at "tmax"'''
    ...
    if verbose: #this just gives us an option to check the solutions
        ...
        plt.subplot(1, 2, 1)
        plt.plot(sol_ss.y[0,:],sol_ss.y[1,:], marker='o', linestyle='dashed', color = 'darkgreen') #sol.y[0,:],sol.y[1,:]
        ...
        plt.xlim(0,17)
        plt.ylim(0,17)
        plt.savefig(str(file_label) + '_path_.svg')
